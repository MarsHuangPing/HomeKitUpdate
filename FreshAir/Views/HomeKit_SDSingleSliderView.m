...
@interface HomeKit_SDSingleSliderView()
{
...
@property (nonatomic, strong) UILabel *lblLeftValue;
@property (nonatomic, strong) UILabel *lblRightValue;
...
@implementation HomeKit_SDSingleSliderView
...
- (void)initUIWithFrame:(CGRect)pFrame
{
    _itemSize = 30;
    _lineHeight = 10;
    _minimumSize = 1;
    ...
    self.backgroundLine.layer.cornerRadius = 5;
    self.backgroundLine.clipsToBounds = YES;
    [self addSubview:self.backgroundLine];
    
    UIImageView *iv_slider_bg = [[UIImageView alloc] initWithFrame:self.backgroundLine.bounds];
    iv_slider_bg.image = [UIImage imageNamed:@"ctrl_slider_bg"];
    [self.backgroundLine addSubview:iv_slider_bg];
    ...
    self.leftLine.clipsToBounds = YES;
    ...
    self.rightLine.clipsToBounds = YES;
    ...
    CGRect rectLeftValue = self.leftCursorButton.frame;
    rectLeftValue.origin.y = rectLeftValue.origin.y - 20;
    rectLeftValue.size.width = 40;
    self.lblLeftValue = [[UILabel alloc] initWithFrame:rectLeftValue];
    self.lblLeftValue.center = CGPointMake(self.leftCursorButton.center.x, self.leftCursorButton.center.y - 20);
    self.lblLeftValue.text = @"0";
    self.lblLeftValue.textAlignment = NSTextAlignmentCenter;
    [self.lblLeftValue setTextColor:[[HomeKit_ColorManager sharedInstance] color2Gray]];
    [self.lblLeftValue setFont:[[HomeKit_FontManager sharedInstance] fontWithSize_13]];
    ...
    if(YES){
        ...
        CGRect rectRightValue = self.rightCursorButton.frame;
        rectRightValue.origin.y = rectRightValue.origin.y - 20;
        rectRightValue.size.width = 40;
        self.lblRightValue = [[UILabel alloc] initWithFrame:rectRightValue];
        self.lblRightValue.center = CGPointMake(self.rightCursorButton.center.x, self.rightCursorButton.center.y - 20);
        self.lblRightValue.text = @"0";
        self.lblRightValue.textAlignment = NSTextAlignmentCenter;
        [self.lblRightValue setTextColor:[[HomeKit_ColorManager sharedInstance] color2Gray]];
        [self.lblRightValue setFont:[[HomeKit_FontManager sharedInstance] fontWithSize_13]];
        [self addSubview:self.lblRightValue];
    }
...
- (void)setDoubleSlider:(BOOL)doubleSlider
{
    if(!doubleSlider){
        CGRect rect = self.rightCursorButton.frame;
        rect.size.width = 0;
        self.rightCursorButton.frame = rect;
        self.rightCursorButton.alpha = 0;
        
        self.lblRightValue.alpha = 0;
    }
}
...
- (void)eventPan:(UIPanGestureRecognizer*)pan
{
    ...
    CGFloat ineffectiveLength = self.itemSize*1;//无效的坐标系长度
    ...
    if (pan.state == UIGestureRecognizerStateEnded) {
        ...
        if(pan.view == self.leftCursorButton){
            ...
            self.lblLeftValue.center = CGPointMake(countOfCalibration*widthOfCalibration+ineffectiveLength/4, pan.view.center.y - 20);
        }else{
            ...
            self.lblRightValue.center = CGPointMake(self.controlWidth - countOfCalibration*widthOfCalibration-ineffectiveLength/4, pan.view.center.y - 20);
        }
        
        if([self.delegate respondsToSelector:@selector(sliderValueEndChangedOfLeft:right:)]){
            [self.delegate sliderValueEndChangedOfLeft:_leftValue right:_rightValue];
...
//            float value = self.doubleSlider?self.itemSize:0;
            ...
            if (pan.view.center.x < self.itemRadius) {
                ...
                self.lblLeftValue.center = CGPointMake(center.x, center.y - 20);
            }
        ...
        self.lblLeftValue.text = [NSString stringWithFormat:@"%ld", [@(_leftValue) integerValue]];
        ...
        if (CGRectGetMinX(self.rightCursorButton.frame) < CGRectGetMaxX(self.leftCursorButton.frame)) {
            ...
            if (pan.view.center.x < self.itemRadius) {
                ...
                self.lblRightValue.center = CGPointMake(center.x, center.y - 20);
            }
            if (pan.view.center.x > CGRectGetWidth(self.bounds)-self.itemRadius) {
                CGPoint center = self.rightCursorButton.center;
                center.x = CGRectGetWidth(self.bounds)-self.itemRadius;
        ...
        self.lblRightValue.text = [NSString stringWithFormat:@"%ld", [@(_rightValue) integerValue]];
        if(self.blockOfValueDidChanged){
            self.blockOfValueDidChanged(_leftValue , _rightValue);
...
- (void)setLeftValue:(double)leftValue
{
    ...
    CGFloat widthOfCalibration = (self.controlWidth-self.itemSize*1)/totalOfCalibration;
...
- (void)setRightValue:(double)rightValue
{
    ...
    NSInteger totalOfCalibration = (self.maxValue - self.minValue)/self.minimumSize - _valueMargin;
    CGFloat widthOfCalibration = (self.controlWidth-self.itemSize*1)/totalOfCalibration;//份长
    ...
    framOfLine.size.width = self.controlWidth - widthOfCalibration*rightValue -  self.itemSize*1;
    if(self.offsetOfAdjustLineEnd){
